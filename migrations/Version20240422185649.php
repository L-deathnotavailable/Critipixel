...
final class Version20240422185649 extends AbstractMigration
{
    ...
    {
        $this->addSql('CREATE TABLE review (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, video_game_id INTEGER NOT NULL, user_id INTEGER NOT NULL, rating INTEGER NOT NULL, comment TEXT DEFAULT NULL, CONSTRAINT FK_794381C616230A8 FOREIGN KEY (video_game_id) REFERENCES video_game (id) ON DELETE CASCADE, CONSTRAINT FK_794381C6A76ED395 FOREIGN KEY (user_id) REFERENCES "user" (id))');
        ...
        $this->addSql('CREATE TABLE video_game (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, title VARCHAR(100) NOT NULL, image_name VARCHAR(255) DEFAULT NULL, image_size INTEGER DEFAULT NULL, slug VARCHAR(255) NOT NULL, description TEXT NOT NULL, release_date DATE NOT NULL --(DC2Type:date_immutable)
        , updated_at DATETIME DEFAULT NULL --(DC2Type:datetime_immutable)
        , test TEXT DEFAULT NULL, rating INTEGER DEFAULT NULL, average_rating INTEGER DEFAULT NULL, number_of_ratings_per_value_number_of_one INTEGER NOT NULL, number_of_ratings_per_value_number_of_two INTEGER NOT NULL, number_of_ratings_per_value_number_of_three INTEGER NOT NULL, number_of_ratings_per_value_number_of_four INTEGER NOT NULL, number_of_ratings_per_value_number_of_five INTEGER NOT NULL)');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_24BC6C50989D9B62 ON video_game (slug)');
        $this->addSql('CREATE TABLE video_game_tags (video_game_id INTEGER NOT NULL, tag_id INTEGER NOT NULL, PRIMARY KEY(video_game_id, tag_id), CONSTRAINT FK_46D6859F16230A8 FOREIGN KEY (video_game_id) REFERENCES video_game (id) ON DELETE CASCADE, CONSTRAINT FK_46D6859FBAD26311 FOREIGN KEY (tag_id) REFERENCES "tag" (id) ON DELETE CASCADE)');
